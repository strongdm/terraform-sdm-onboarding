...
This script will, or can potentially, create and then destroy all of these categories of items, so for the purposes of this demo, you might grant the key access to all of these items. There is an option to set the key to expire so that you do not forget to remove it later, if you wish.
...
* You will be required to enter your AWS credentials and SDM credentials when running Terraform commands with them, so keep those available.
...
All right! We have a working strongDM setup now, let's get the client installed, and test our connection and ability to query our resources!
